...
public class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	protected void openAnimate(int position) {
		final View childAt = swipeListView.getChildAt(position - swipeListView.getFirstVisiblePosition());
		if (childAt == null) return;
		openAnimate(childAt.findViewById(swipeFrontView), position);
	}
	...
	protected void closeAnimate(int position) {
		final View childAt = swipeListView.getChildAt(position - swipeListView.getFirstVisiblePosition());
		if (childAt == null) return;
		closeAnimate(childAt.findViewById(swipeFrontView), position);
	}
	 ...
	 * @param swapRight
	 *            If swap is true, this parameter tells if move is to the right or left
	 * @param position
	 *            Position of list
	 */
	 ...
	 * @param swapRight
	 *            If swap is true, this parameter tells if movement is toward right or left
	 * @param position
	 *            list position
	 */
	...
	public void move(float deltaX) {
		...
		if (swipeCurrentAction == SwipeListView.SWIPE_ACTION_DISMISS) {
			...
			if ((swipingRight && deltaX > 0 && posX < DISPLACE_CHOICE) || (!swipingRight && deltaX < 0 && posX > -DISPLACE_CHOICE)
